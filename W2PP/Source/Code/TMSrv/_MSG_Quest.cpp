...
#pragma region CARBUNCLE_WIND
	case CARBUNCLE_WIND:
	{
		...
		if (pMob[npcIndex].MOB.SkillBar[0] != 255)
			SetAffect(conn, 41, 400, 100); //n�mero da skill no skilldata, tempo de dura��o(400 da 128 segundos), level da skill
		if (pMob[npcIndex].MOB.SkillBar[1] != 255)
			SetAffect(conn, 43, 400, 100); //n�mero da skill no skilldata, tempo de dura��o(400 da 128 segundos), level da skill
		if (pMob[npcIndex].MOB.SkillBar[2] != 255)
			SetAffect(conn, 44, 400, 100); //n�mero da skill no skilldata, tempo de dura��o(400 da 128 segundos), level da skill
		if (pMob[npcIndex].MOB.SkillBar[3] != 255)
			SetAffect(conn, 45, 400, 100); //n�mero da skill no skilldata, tempo de dura��o(400 da 128 segundos), level da skill
...
#pragma endregion
#pragma region GUARDA_REAL_EVT
	case GUARDA_REAL_EVT:
	{
		int ItemReq = 4056; // ID do item exigido para a troca
		...
		if (i != pMob[conn].MaxCarry)
		{
			...
			if (_rd < 25)
				Item.sIndex = 413; // primeiro item de troca
			else if (_rd < 50)
				Item.sIndex = 412; // segundo item de troca
			else if (_rd < 75)
				Item.sIndex = 413; // terceiro item de troca
			else
				Item.sIndex = 412; // quarto item de troca

			sprintf(temp, g_pMessageStringTable[_SN_Item_Arrived], g_pItemList[Item.sIndex].Name);
			SendClientMessage(conn, temp);

			PutItem(conn, &Item);
			DoTeleport(conn, 1256, 2126); // destino após a entrega do prêmio
			break;
		}
...
#pragma region CURANDEIRO
	case CURANDEIRO:
	{
		//Prote��o para impedir o uso fora da cidade e fora das coordenadas especificadas By Faelz'
		...
		//Fim Prote��o para impedir o uso fora da cidade e fora das coordenadas especificadas By Faelz'
