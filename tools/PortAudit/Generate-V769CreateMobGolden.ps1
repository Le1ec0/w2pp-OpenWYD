...
$probeSource = Join-Path $PSScriptRoot 'V769CreateMobGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\create-mob.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-create-mob-golden-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "7.69 CreateMob golden probe failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 CreateMob golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{448}$') { throw "Unexpected 7.69 CreateMob payload: expected 448 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 CreateMob payload fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-create-mob-golden-', [System.StringComparison]::Ordinal)) {
