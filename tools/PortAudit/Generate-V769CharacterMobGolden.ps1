...
$probeSource = Join-Path $PSScriptRoot 'V769CharacterMobGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\character-mob.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-character-mob-golden-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "7.69 character-MOB golden probe compilation failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 character-MOB golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{2080}$') { throw "Unexpected 7.69 MOB payload: expected 2080 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 character-MOB fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-character-mob-golden-', [System.StringComparison]::Ordinal)) {
