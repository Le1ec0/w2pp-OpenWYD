...
$probeSource = Join-Path $PSScriptRoot 'V769UpdateEtcGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\update-etc.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-update-etc-golden-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "7.69 UpdateEtc golden probe failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 UpdateEtc golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{72}$') { throw "Unexpected 7.69 UpdateEtc payload: expected 72 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 UpdateEtc payload fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-update-etc-golden-', [System.StringComparison]::Ordinal)) {
