...
$probeSource = Join-Path $PSScriptRoot 'V769AccountLoginGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\account-login.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-account-login-golden-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "7.69 account-login golden probe compilation failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 account-login golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{3832}$') { throw "Unexpected 7.69 account-login payload: expected 3832 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 account-login payload fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-account-login-golden-', [System.StringComparison]::Ordinal)) {
