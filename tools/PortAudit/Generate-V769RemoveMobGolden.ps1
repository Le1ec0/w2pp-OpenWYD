...
$probeSource = Join-Path $PSScriptRoot 'V769RemoveMobGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\remove-mob.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-remove-mob-golden-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "7.69 RemoveMob golden probe failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 RemoveMob golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{8}$') { throw "Unexpected 7.69 RemoveMob payload: expected 8 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 RemoveMob payload fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-remove-mob-golden-', [System.StringComparison]::Ordinal)) {
