...
$probeSource = Join-Path $PSScriptRoot 'V769CharacterLoginGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\character-login.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-character-login-golden-' + [guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $tempDirectory | Out-Null
try {
    $objectFile = Join-Path $tempDirectory 'probe.obj'
    $executable = Join-Path $tempDirectory 'probe.exe'
    $compile = 'call "{0}" x86 >nul && cl.exe /nologo /EHsc /std:c++17 /W0 /I"{1}" /Fo"{2}" /Fe"{3}" "{4}" >nul' -f $vcvars, $includeRoot, $objectFile, $executable, $probeSource
    & $env:ComSpec /d /s /c $compile
    if ($LASTEXITCODE -ne 0) { throw "7.69 character-login golden probe compilation failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 character-login golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{3432}$') { throw "Unexpected 7.69 character-login payload: expected 3432 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 character-login payload fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-character-login-golden-', [System.StringComparison]::Ordinal)) {
