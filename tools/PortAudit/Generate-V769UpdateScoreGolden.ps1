...
$probeSource = Join-Path $PSScriptRoot 'V769UpdateScoreGoldenProbe.cpp'
$fixturePath = Join-Path $workspaceRoot 'Server\tests\WydCdk.Engine.Tests\Fixtures\V769\update-score.golden.hex'
...
$tempDirectory = Join-Path $PSScriptRoot ('.v769-update-score-golden-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "7.69 UpdateScore golden probe failed with exit code $LASTEXITCODE." }

    $hex = (& $executable | Out-String).Trim()
    if ($LASTEXITCODE -ne 0) { throw "7.69 UpdateScore golden probe failed with exit code $LASTEXITCODE." }
    if ($hex -notmatch '^[0-9A-Fa-f]{280}$') { throw "Unexpected 7.69 UpdateScore payload: expected 280 hex characters, received $($hex.Length)." }
    ...
    Write-Output "Generated 7.69 UpdateScore payload fixture: $fixturePath"
}
finally {
    ...
    if ($resolvedTemp.StartsWith($resolvedToolRoot, [System.StringComparison]::OrdinalIgnoreCase) -and
        (Split-Path -Leaf $resolvedTemp).StartsWith('.v769-update-score-golden-', [System.StringComparison]::Ordinal)) {
